...
def export_optimizer_top_configs(results: List[Dict], filename: str, top_n: int = 10) -> None:
    if not results:
        ...
        return
    results_sorted = sorted(results, key=lambda x: x.get("test_final_balance", 0), reverse=True)
    top_results = results_sorted[:top_n]
    keys = top_results[0].keys()
    with open(filename, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=keys)
        writer.writeheader()
        writer.writerows(top_results)
    print(f"Top {top_n} optimizer configs exported to {filename}")
